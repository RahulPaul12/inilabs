...
const BuiltWayofWork = () => {
    ...
    const AboutTab =()=> <div className=" p-4 bg-white rounded-lg md:p-8 dark:bg-gray-800" id="about" role="tabpanel" aria-labelledby="about-tab">
...
const ServiceTab =()=> <div className=" p-4 bg-white rounded-lg md:p-8 dark:bg-gray-800" id="services" role="tabpanel" aria-labelledby="services-tab">
...
const StatiSticsTab = ()=>  <div className=" p-4 bg-white rounded-lg md:p-8 dark:bg-gray-800" id="statistics" role="tabpanel" aria-labelledby="statistics-tab">
...
<ul className="flex flex-wrap text-sm font-medium text-center text-gray-500 border-b border-gray-200 rounded-t-lg bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:bg-gray-800" id="defaultTab" data-tabs-toggle="#defaultTabContent" role="tablist">
   
   {
    actions.map(action=>(
        <li className="me-2">
            <button id={action.type} onClick={(event)=>{setContent(action.type) ,  setIsActive(true), console.log(event.currentTarget.id, content) 
                event.currentTarget.id ==  content? setIsActive(true) : setIsActive(false)

                }
                }  type="button" role="tab"   className={`inline-block p-4 ${isActive?"text-blue-600" : "text-black"}  rounded-ss-lg `}>{action.name}</button>
            </li>
    ))
   }
